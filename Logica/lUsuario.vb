...
Public Class lUsuario
    ...
    Public Function listar_Usuario() As DataSet
        ...
        con.Conectar()
        Dim comando As New SqlDataAdapter("SELECT * FROM vistaMostrarUsuarios WHERE USU_ESTA = 'A' ORDER BY USUARIO", con.strConex)
    ...
    Public Function registrar_usuario() As Boolean
        Try
            ...
            Dim data As Byte() = File.ReadAllBytes(USU_IMAGEN)
    ...
    Public Function modificar_usuariob() As Boolean
        Try
            con.Conectar()

            Dim comando = New SqlCommand("proc_ModificarUsuariob")
            comando.CommandType = CommandType.StoredProcedure
            comando.Connection = con.strConex
            comando.Parameters.AddWithValue("@codigo", USU_DNI)
            comando.Parameters.AddWithValue("@nombre", USU_NOM)
